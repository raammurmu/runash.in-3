...
export default function RunAshChatPage() {
  ...
  const performSendMessage = async ({
    ...
    if (optimistic) {
      setMessages((prev) => (prev.some((message) => message.id === userMessageId) ? prev : [...prev, userMessage]))
    }
    ...
    try {
      ...
      if (DEV_CHAT_FALLBACK_ENABLED) {
        const fallback = generateAIResponse(content, activeSessionId, requestId)
  ...
  const handleRetryLastMessage = async () => {
    ...
    setMessages((prev) => prev.filter((message) => message.id !== chatError.fallbackMessageId))
  ...
  const generateAIResponse = (userInput: string, sessionId: string, requestId: string): ChatMessage => {
