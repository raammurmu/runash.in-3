...
class InMemoryJobQueue {
  ...
  replayDeadLetter(jobId: string): QueueJob | null {
    ...
    const replayed = this.deadLetterJobs.splice(idx, 1)[0]
    replayed.status = "queued"
    replayed.attempts = 0
    replayed.lastError = undefined
    replayed.updatedAt = new Date().toISOString()

    const existingJob = this.jobs.find((job) => job.id === replayed.id)
    if (existingJob) {
      existingJob.status = "queued"
      existingJob.attempts = 0
      existingJob.lastError = undefined
      existingJob.updatedAt = replayed.updatedAt
    } else {
      this.jobs.push(replayed)
    }
